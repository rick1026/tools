...
pcap_t *pcap_open_hdl = pcap_open_live("eth2", 65535, 1, 5000, errBuf);
// pcap_set_buffer_size( pcap_open_hdl,20480000);
//pcap_setbuff(pcap_open_hdl,20480000);
      ...
      //  list_for_each(p, &list_capture_intf)
        {
          ...
          if ((poll_pcap_fd[i].revents & POLLIN) != 0)
            ...
            //pcap_dispatch(pcap_open_handle[i], 1, packet_processer, (unsigned char*)entry->intf_name);
              pcap_dispatch(pcap_open_handle[i], 1, packet_processer, (unsigned char*)("eth2"));
			}
		  i++;
        }
      }
