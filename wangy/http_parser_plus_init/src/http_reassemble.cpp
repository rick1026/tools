...
char cls_http_reassemble::do_http_reassemble(stru_hash_node *cur_node,TCPsessionID  sess_id,
                                             unsigned int  sdu_len, unsigned char *sdu_data,
                                             stru_http_req_head *http_header, enum TCPDirection dir,
                                             unsigned int  *http_data_len, unsigned char **http_data_buf, int *http_type)
{
    ...
    switch(http_obj->status)
    {
        ...
        case S_RSP_HEAD_END:
	    	...
	    	{
                if ((sdu_len >= 5) && !memcmp(sdu_data + sdu_len - 5, "0\r\n\r\n", 5))
                {
#ifdef __HTTP_REASSEMBLE_LOG
...
inline int cls_http_reassemble::insert_list(stru_tcpsdu_node **head,unsigned char *data,int data_len)
{
    if(data_len <= 0)
        return 0;
...
inline int cls_http_reassemble::get_list_data(stru_tcpsdu_node *head,unsigned char *data)
{
    int pos = 0;
    while(head != NULL)
    {
...
inline int cls_http_reassemble::free_list(stru_tcpsdu_node **head)
{
