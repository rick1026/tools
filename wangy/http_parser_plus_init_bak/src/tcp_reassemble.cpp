...
#if 0
  for (i = 0; i < sizeof(TCPsessionID); i += 2)
    ret += *(unsigned short*)((unsigned short*)(key + i));
#endif
...
char cls_tcp_reassemble::do_tcp_reassemble(stru_hash_node *cur_node,unsigned int src_ip, unsigned int dst_ip,
    ...
    unsigned int *data_len,unsigned char **data_buf)
{
  ...
  printf("process_tcp_packet: find_hash from tcp_reassemble_tbl by tcp_id\n");
	...
	if(cur_node == NULL)
		return TCP_SEG_RECVING ;

  //pBucket = find_hash(tcp_reassemble_tbl, &tcp_id);
  if(cur_node->tcp_node == NULL)
  {
#ifdef __TCP_REASSEMBLE_LOG
	...
	cur_node->tcp_node = p_tcp_session;
  }
  ...
  else
  {
	...
	p_tcp_session = cur_node->tcp_node;
  ...
  if(to_continue)
  {
    ...
    if((head->flags & TCP_FLAG_RST) != 0)
    {
      ...
      //free(pBucket);
		free(cur_node->tcp_node);
		cur_node->tcp_node = NULL;
                          ...
                          p_tcp_session->state == FIN_WAIT_2 ))
    {
      ...
      if (p_tcp_session->forward_min_got == 0 && tcp_id.src_ip == cur_node->sess_id.src_ip)
      {
#ifdef __TCP_REASSEMBLE_LOG
      ...
      if(p_tcp_session->backward_min_got == 0 && tcp_id.src_ip != cur_node->sess_id.src_ip)
      {
#ifdef __TCP_REASSEMBLE_LOG
      ...
      // end added
	//printf("ip1 = %u,ippppppppppppppppppppppppppppppp%u\n",tcp_id.src_ip,cur_node->sess_id.src_ip);
      if(tcp_id.src_ip == cur_node->sess_id.src_ip)
      {
#ifdef __TCP_REASSEMBLE_LOG
    ...
    {
    case    SYN_SENT:
      if(tcp_id.src_ip == cur_node->sess_id.dst_ip &&
      ...
      if ((tcp_id.src_ip == cur_node->sess_id.src_ip) && ((head->flags & TCP_FLAG_ACK) == 0))
      {
        p_tcp_session->init_backward_seq = head->ack_num;
      }
      break;
