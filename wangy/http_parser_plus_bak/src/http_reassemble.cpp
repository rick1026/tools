...
char
cls_http_reassemble::FreeHttpHashNode (TCPsessionID sess_id)
{
...
cls_http_reassemble::cls_http_reassemble ()
{
  ...
  if ((http_reassemble_tbl =
    ...
    {
      printf
	("cls_http_reassemble: create_hash(http_reassemble_tbl) error, exit!\n");
      exit (1);
    }
...
void *
cls_http_reassemble::http_cache_tbl_timer (void *arg)
{
  cls_http_reassemble *pThis =
    reinterpret_cast < cls_http_reassemble * >(arg);
  pThis->do_timer ();
}

void
cls_http_reassemble::do_timer ()
{
  while (1)
    {
      sleep (5);
      printf
	("%u, cls_http_reassemble do_timer() running, addr of http_reassemble_tbl = %p!!!\n",
...
char
cls_http_reassemble::do_http_reassemble (TCPsessionID sess_id,
...
inline int
cls_http_reassemble::insert_list (stru_tcpsdu_node ** head,
...
inline int
cls_http_reassemble::get_list_data (stru_tcpsdu_node * head,
				    unsigned char *data)
{
  int pos = 0;
  while (head != NULL)
...
inline int
cls_http_reassemble::free_list (stru_tcpsdu_node ** head)
{
...
char
cls_http_reassemble::analyse_http_req (unsigned char *data,
...
char
cls_http_reassemble::analyse_http_resp (unsigned char *data,
					unsigned int datalen,
					stru_http_req_head * header)
{
  // 缓冲区为空或者长度过小,错误返回
...
int
cls_http_reassemble::dechunk_data (unsigned char *data,
...
int
cls_http_reassemble::unzip_data (unsigned char *data, unsigned int data_len,
...
int
cls_http_reassemble::atox (char *src)
{
  unsigned int dst = 0;
  int len = 0;
...
char
cls_http_reassemble::beap_uncompress_new (unsigned char *uncompr,
...
#define MAX_UNZIP_SIZE 320000
    ...
    while(inits_num <= 4)
    {
        ...
        if (inits_num == 4 && err != Z_OK && err != Z_STREAM_END)
        {
            break;
        }
        if (err == Z_OK || err == Z_STREAM_END)
        {
            *uncompr_len = MAX_UNZIP_SIZE - stream.avail_out;
            inflateEnd(&stream);
            return 0;
        }
        if (err == Z_BUF_ERROR)
