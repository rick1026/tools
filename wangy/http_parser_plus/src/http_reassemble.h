...
public:
    ...
    char do_http_reassemble(TCPsessionID  sess_id,
   	...
   	char FreeHttpHashNode(TCPsessionID sessid); 
	...
	char beap_uncompress(unsigned char *uncompr, \
                       unsigned int  *uncompr_len, \
                       unsigned char *compr, \
                       unsigned int  *compr_len);

	int ungzip(unsigned char *des, unsigned int *des_len, unsigned char* source,unsigned int len);
};
