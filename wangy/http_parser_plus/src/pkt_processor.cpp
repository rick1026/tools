...
extern unsigned long long tm_processer;
        ...
        for (size_t j = 0; j < len_rule; j++)
        {
            ...
            switch(temp_rule.Rule_SubType)
            {
                ...
                case 3:
					...
					//memcpy(keyword, temp_rule.Rule_Keyword+(strlen((const char *)(temp_rule.Rule_Keyword))-12), 12);
					//printf("keyword = [%s]~[%s],len = %d\n",keyword,strlen(keyword));
...
struct timeval t2;
	...
	unsigned char temp = search_rslt[i];
		...
		stru_prule_position coordinate= {result_info_list[i].rule_id, result_info_list[i].key_id};
		list_rule_bingo.push_back(coordinate);
//		printf("rule_id = %d, key_id = %d, bingo keyword: %s\n", result_info_list[i].rule_id, result_info_list[i].key_id, result_info_list[i].keyword);
...
void cls_pkt_processor::do_radom_work(void* arg)
{
    ...
    {
    	sleep(5);
		printf("################packet_num=%u,http_num=%d, tm_processer: %llu\n",cap_pkt_cnt,http_num, tm_processer);
...
begin:
    ...
    ip_head_len = (struL3Head.version & 0x0f) << 2;
	...
	/*
     if(struL3Head.protocol == 0x2f)
     {
     pPacket_begin = (unsigned char*)packet + 28;
     goto begin;
     } 
	*/
    ...
    switch (struL3Head.protocol)
    {
        case  TCP_PROTO:
            ...
            //tcp_head_len = ((struTCPHead.header_len & 0xf0) >> 4) << 2;
            tcp_head_len = ((struTCPHead.header_len & 0xf0) >>  2);
    ...
    sport = struTCPHead.source_port;
    dport = struTCPHead.dest_port;
    ...
    if (content_len == 0 && (((struL3Head.flag&0x4)!=0x4)||((struL3Head.flag & 0x1) != 0x1)))
    {
#ifdef __PRINT_LOG
