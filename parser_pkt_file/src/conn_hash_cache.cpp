...
cls_conn_hash_cache::cls_conn_hash_cache()
{
    ...
    printf("cls_conn_hash_cache(): addr of cache_conn_hash = %p\n", cache_conn_hash);
...
stru_conn_node* cls_conn_hash_cache::do_conn_hash_cache(TCPsessionID sess_id)
{
...
void cls_conn_hash_cache::insert_not_text_to_conn(TCPsessionID sess_id, stru_http_header http_head, ENUM_PKT_ACTION action)
{
#ifdef __CONN_HASH_CACHE_LOG
    ...
    p_bucket->content = (void*)p_conn;
    p_bucket->key = &(p_conn->sess_id);

    insert_hash(cache_conn_hash, p_bucket);
}

void cls_conn_hash_cache::insert_text_to_conn(TCPsessionID sess_id, stru_http_header http_head, ENUM_PKT_ACTION action)
{
    hash_bucket *p_bucket = (hash_bucket*)calloc(1, sizeof(hash_bucket));
    if (p_bucket == NULL)
        return;
...
stru_conn_node *cls_conn_hash_cache::insert_new_conn(TCPsessionID sess_id, stru_http_header http_head)
{
    hash_bucket *p_bucket = (hash_bucket*)calloc(1, sizeof(hash_bucket));
    if (p_bucket == NULL)
        return NULL;
